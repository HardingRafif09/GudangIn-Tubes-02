...
func tambahBarang(A *DaftarBarang, n *int) {
	...
	if cariBarang(*A, *n, kode) != -1 {
		fmt.Println("Kode sudah digunakan! Gunakan kode yang berbeda.")
		return
	}

	A[*n].Kode  = kode
	A[*n].Nama  = readNonEmpty("Nama Barang  : ") // FIX #8
	A[*n].Harga = readPositiveInt("Harga Barang : RP. ") // FIX #6
	A[*n].Stok  = readPositiveInt("Stok Barang  : ") // FIX #6
	*n++
...
func catatTransaksi(gudang *DaftarBarang, jmlBarang int, trx *DaftarTransaksi, jmlTrx *int) {
	...
	jenisRaw := readNonEmpty("Jenis Transaksi (Masuk / Keluar) : ")
	jenis    := strings.Title(strings.ToLower(jenisRaw))
...
func main() {
	...
	for {
		...
		case 3:
			// FIX #4: kirim flag kode, bukan stok
